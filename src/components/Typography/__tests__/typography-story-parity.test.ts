...
import reactMeta, * as reactStories from '../Typography.stories';
import vueMeta, * as vueStories from '../../../../storybooks/vue/src/stories/Typography.stories';
...
describe('Typography story parity', () => {
  ...
  it('keeps Storybook metadata aligned across React and Vue', () => {
    ...
    expect(react.title).toBe('Atomics/Typography');
    expect(react.id).toBe('atomics-typography-react');
    expect(vue.id).toBe('atomics-typography-vue');
  });
