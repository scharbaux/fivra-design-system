...
import reactMeta, * as reactStories from '../Box.stories';
import vueMeta, * as vueStories from '../../../../storybooks/vue/src/stories/Box.stories';
...
describe('Box story parity', () => {
  ...
  it('keeps Storybook metadata aligned across React and Vue', () => {
    ...
    expect(react.title).toBe('Atomics/Box');
    expect(react.id).toBe('atomics-box-react');
    expect(vue.id).toBe('atomics-box-vue');
  });
