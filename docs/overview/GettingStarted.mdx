...
> **Tip:** Aligning with the CI toolchain prevents mismatched lockfiles or Storybook builder warnings. See the [tech stack overview](./TechStack.mdx) for the full dependency matrix.
...
- `generate:tokens` runs the design token transformer described in the [design token workflow](./TechStack.mdx#design-token-theme-workflow) so the Engage and Legacy themes stay in sync with the docs.
...
> Override refs with environment variables when consuming remote builds: set `STORYBOOK_ANGULAR_URL` or `STORYBOOK_VUE_URL` before running the command. The [multi-framework composition guide](./ComponentArchitecture.mdx#multi-framework-storybook-composition) dives deeper into the topology.
...
- Review framework-specific implementation notes in the [Component Architecture](./ComponentArchitecture.mdx) guide.
